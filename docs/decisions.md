...
- **Decision:** Three behavioural-state scalars per NPC, with deliberately asymmetric properties:
  ...
  - **Patience** (`0.0` to `1.0`, per-conversation). **NOT persisted.** Runtime-only. On engage, resets to `clampf(1.0 - 0.25 * low_patience_dismissals, 0.0, 1.0)` — accumulated burnout from prior dismissals carries across conversations via this multiplicative penalty (refined 2026-05-10 after M11 playtest; original spec was a hard reset to 1.0). Updated rule-based per turn (insult `-0.25`, repeated question `-0.10`, default `-patience_decay_rate`, compliment `+0.10`, aligned-interest `+0.05`).
...
- **Reason:** Mixing mechanisms is correct because the three scalars answer three different questions. Mood ("how is this NPC feeling generally") and valence ("how does this NPC feel about that entity") evolve slowly over conversations and benefit from LLM contextual judgment — only the model can tell whether "thanks" was sincere or sarcastic. Patience ("is this player wearing this NPC out *right now*") is a per-conversation signal that needs deterministic per-turn response so insults reliably trigger NPC-initiated farewell — round-tripping every turn through the LLM is too slow and too costly. Persisting patience would defeat its purpose (the next conversation should reset, otherwise an NPC who got angry yesterday starts at -0.4 today, which is what valence is for). NL band words consistently outperform raw scalars on a 3B model — "warm" guides voice choice better than "0.4" on a model that doesn't reliably reason about scalars. **Patience-starting-point refinement (2026-05-10):** the original "resets to 1.0 on engage" lock assumed M10's LLM-driven `valence_delta` would carry burnout between conversations via trust — an NPC the player had burnt out four times would start cold because their valence had drifted negative. M11 playtest revealed the gap: with patience hard-resetting to 1.0 and valence still at default 0 (M10 not yet shipped), burnout evaporated between engagements. The `low_patience_dismissals` counter was being written but not read by any current-conversation behaviour. The refined rule (`1.0 - 0.25 * dismissals`) bridges this until M10 lands: starting patience reflects history, the patience-word band shifts down ("engaged" → "fading" → "worn out") so Raebai's voice carries the burnout from the very first reply, and burnout snowballs across sessions ("memory is important"). When M10 ships, this becomes derive-from-trust instead of derive-from-counter — same UX, more durable plumbing.
- **Alternatives considered:**
  ...
  - **Hard reset to 1.0 on engage** (original 2026-05-09 lock) — refined after M11 playtest: burnout from prior dismissals must carry until M10 wires valence to do it. See Reason above.
- **Status:** Locked at the refined patience-starting-point shape.
- **Date locked:** 2026-05-09. **Updated 2026-05-10** with the burnout-carry refinement.
...
## D-042 — NPC-initiated farewell: patience-triggered addendum + force-close (regex gate removed)

- **Decision:** D-037's deferred half (NPC chooses to end the conversation) lands via patience: when an NPC's patience drops below `0.2` at the start of their next turn, the next dialogue request receives a one-shot system addendum telling the LLM **this is its final reply** and to produce a brief parting line in character. When the reply lands, the engine instant-closes the UI **regardless of the specific phrasing** — patience itself is the trigger, the NPC's reply IS the parting line. The `low_patience_dismissals` counter in `relationships.player` increments so repeated dismissals can shape trust at consolidation time.
- **Reason:** NPC-initiated farewell needs a stable trigger that's contextually meaningful. Patience supplies the signal (this player is wearing this NPC out, computed cheaply per turn). The asymmetry with D-037 (player-side) is intentional: D-037's regex works because we *control the player's vocabulary* — a finite set of acceptable parting phrases. The NPC's reply is LLM-generated and can phrase a dismissal a thousand ways ("Hm. Let me sit with my books", "Door's that way, duck", "I think we're done here") — a regex gate on the reply was tried in M11 and failed in playtest because Llama 3.2 3B consistently produced in-character dismissals using vocabulary the D-037 regex didn't catch. The pivot: patience is the gate, the addendum is directive enough that the LLM produces a parting line, and the engine doesn't need to grade phrasing afterward. The `low_patience_dismissals` counter feeds back into trust slowly through consolidation — a player who repeatedly burns out NPCs sees their valence drift down without a per-turn rule that would feel like a slot machine.
- **Alternatives considered:**
  - **Regex-on-NPC-reply gate** (original 2026-05-09 lock; tried in M11) — rejected after playtest. The reply often wrapped up the conversation naturally ("Let me be") without using D-037's narrow vocabulary; the gate left the UI open after a clear dismissal. Symmetric-with-D-037 detection sounded clean on paper but fights how LLMs phrase parting lines.
  ...
  - **Wait for the LLM to phrase the farewell, then check for it** (no patience trigger, just always run the regex) — rejected; would create false positives on phrases like "see what you've done" — the patience gate is what makes the farewell intentional.
- **Status:** Locked at the updated shape (force-close on patience trigger; no regex gate). Implementation in M11.
- **Date locked:** 2026-05-09. **Updated 2026-05-10** after M11 playtest revealed the regex-gate was the wrong primitive.
