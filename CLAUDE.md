...
- **What changed:**
  ...
  - **M9 Task 9 verification + integration bug** (this session, 2026-05-10):
    - Restored the original v0.0 v1 save (Coldrose/Raaed/dragons history, 22 messages) over the live save as the migration test seed. Snapshotted the existing pre-M9 v2 dev-test save first (it was only 4 messages of dev chatter — first hint that something was wiping saves on every run).
    ...
    - **Re-verified after fix:** v1 → v2 migration still works; engage + fresh turn + farewell + save persists 30 messages; quit + relaunch + re-engage reproduces the magic moment with name "Raaed" and on-prompt recall of "Coldrose and dragons". Save survived the reload. Boot is clean, zero errors, zero warnings.
