...
func _draw() -> void:
	...
	for i in _count:
		...
		if age < 0.0 or age > SLASH_VISIBLE:
			continue
		var intensity: float = 1.0 - age / SLASH_VISIBLE
		var t: float = _hit_at[i] / LIFE
		var off: float = (t - 0.5) * 1.5           # fan the slashes across the arc
		var r: float = RANGE * (0.5 + 0.5 * t)
		draw_arc(_origin, r, ang - 1.0 + off, ang + 1.0 + off, 14, Color(1.4, 1.2, 1.7, 0.9 * intensity), 3.0, true)
		draw_arc(_origin, r * 1.06, ang - 0.8 + off, ang + 0.8 + off, 14, Color(_color.r, _color.g, _color.b, 0.5 * intensity), 1.5, true)
