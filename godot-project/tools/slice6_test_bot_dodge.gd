...
func _process(_delta: float) -> bool:
	...
	_ran = true
	var failed: int = 0
	failed += _test_projectile()
	failed += _test_circle()
	failed += _test_lane()
	failed += _test_safety()
	failed += _test_ladder()
	failed += _test_reactions()
	if failed > 0:
		printerr("Bot dodge tests: %d FAILED" % failed)
		quit(1)
	else:
...
func _test_projectile() -> int:
	...
	ok += _expect(bool(head_on["threatening"]), "a bolt aimed dead-on is a threat")
	ok += _expect(absf(float(head_on["tti"]) - 0.6667) < 0.02,
		"time-to-impact solves to distance/speed (got %f)" % head_on["tti"])
	ok += _expect(float(head_on["exit_len"]) > 0.0, "a dead-on bolt yields a real exit distance")
	...
	ok += _expect(not bool(wide["threatening"]), "a bolt passing 60 px wide is not a threat")
	...
	ok += _expect(not bool(away["threatening"]), "a bolt already past me is not a threat")
	ok += _expect(is_equal_approx(float(away["t"] if away.has("t") else away["tti"]), 0.0),
		"a receding bolt clamps time-to-impact to 0")
	# The exit is SIDEWAYS out of the lane, not backwards down it.
	ok += _expect(absf((head_on["exit"] as Vector2).y) > absf((head_on["exit"] as Vector2).x),
		"exit from a horizontal bolt is vertical (step off the lane)")
	return ok


func _test_circle() -> int:
	var ok: int = 0
	...
	ok += _expect(bool(caught["threatening"]), "standing inside a marked circle is a threat")
	ok += _expect((caught["exit"] as Vector2).x > 0.0, "exit runs directly away from the centre")
	ok += _expect(float(caught["exit_len"]) > 90.0 - 20.0 - 1.0,
	...
	ok += _expect(not bool(clear["threatening"]), "standing well outside the circle is safe")
	...
	ok += _expect(not bool(leaving["threatening"]),
	...
	ok += _expect(bool(dead["degenerate"]), "dead-centre is reported as degenerate")
	ok += _expect(bool(dead["threatening"]), "dead-centre is still a threat")
	return ok
...
func _test_lane() -> int:
	...
	ok += _expect(bool(inside["threatening"]), "a point inside the lane is a threat")
	ok += _expect((inside["exit"] as Vector2).y > 0.0, "exit is perpendicular, toward the near side")
	...
	ok += _expect(not bool(beside["threatening"]), "a point beside the lane is safe")
	...
	ok += _expect(not bool(past["threatening"]), "a point beyond the lane length is safe")
	# Behind the origin.
	var behind: Dictionary = BotDodge.threat_from_lane(
		Vector2(-30.0, 0.0), Vector2.ZERO, Vector2.ZERO, 0.0, 200.0, 40.0, 0.4)
	ok += _expect(not bool(behind["threatening"]), "a point behind the lane origin is safe")
	...
	ok += _expect(bool(rot["threatening"]), "a rotated lane still contains its centreline")
	return ok


func _test_safety() -> int:
	var ok: int = 0
	var me := Vector2.ZERO
	...
	ok += _expect(pick == right, "never dodge into a pit (picked %s)" % pick)
	...
	ok += _expect(pick2 == left, "never dodge into another live telegraph (picked %s)" % pick2)
	...
	ok += _expect(pick3 == Vector2.ZERO, "no safe exit is reported honestly")
	# Region containment for a lane shape.
	ok += _expect(BotDodge.point_in_region(Vector2(50.0, 0.0),
		{"shape": "line", "from": Vector2.ZERO, "to": Vector2(200.0, 0.0), "width": 40.0}),
		"lane region contains its centreline")
	ok += _expect(not BotDodge.point_in_region(Vector2(50.0, 200.0),
		{"shape": "line", "from": Vector2.ZERO, "to": Vector2(200.0, 0.0), "width": 40.0}),
		"lane region excludes a far point")
	return ok


func _test_ladder() -> int:
	var ok: int = 0
	var threat := {"threatening": true, "tti": 0.4, "exit": Vector2(0.0, -50.0), "exit_len": 50.0}
	# Dash clears 87 px > 50 px needed -> dash wins outright.
	var r1: Dictionary = BotDodge.choose_response(threat,
		{"dash_ready": true, "dash_dist": 87.0, "grounded": true})
	ok += _expect(String(r1["action"]) == "dash", "prefers a dash that clears the region")
	...
	ok += _expect(String(r2["action"]) == "jump", "falls to a jump for a vertical exit")
	...
	ok += _expect(String(r3["action"]) == "walk", "falls to walking out")
	...
	ok += _expect(String(r4["action"]) == "blink", "blinks when the dash cannot reach")
	...
	ok += _expect(String(r5["action"]) == "parry", "parries a hit landing inside the window")
	var late := {"threatening": true, "tti": 0.6, "exit": Vector2(150.0, 0.0), "exit_len": 150.0}
	var r6: Dictionary = BotDodge.choose_response(late,
		{"can_parry": true, "parry_ready": true, "parry_window": 0.16})
	ok += _expect(String(r6["action"]) != "parry", "does not parry a hit that is still far off")
	...
	ok += _expect(String(r7["action"]) != "dash_iframe", "i-frame dash is off by default")
	var r8: Dictionary = BotDodge.choose_response(imminent,
		{"dash_ready": true, "dash_dist": 87.0, "allow_iframe": true})
	ok += _expect(String(r8["action"]) == "dash_iframe", "i-frame dash unlocks at the top tiers")
	# Not threatening -> do nothing at all.
	var r9: Dictionary = BotDodge.choose_response({"threatening": false}, {"dash_ready": true})
	ok += _expect(String(r9["action"]) == "none", "no threat means no reaction")
	return ok


func _test_reactions() -> int:
	var ok: int = 0
	var r := BotDodge.Reactions.new()
	r.observe(1, 0.0, 0.25)
	ok += _expect(not r.visible(1, 0.24), "a threat is invisible before the reaction delay")
	ok += _expect(r.visible(1, 0.26), "a threat becomes visible after the reaction delay")
	# Re-observing must not restart the clock, or the bot never reacts.
	r.observe(1, 0.30, 0.25)
	ok += _expect(r.visible(1, 0.31), "re-sighting does not reset an elapsed reaction")
	...
	ok += _expect(not w.visible(2, 5.0), "a whiffed threat stays unreacted-to")
	w.observe(3, 0.0, 0.1, 0.99, 0.45)      # roll above p_miss -> react normally
	ok += _expect(w.visible(3, 0.2), "a non-whiffed threat is reacted to")
	# Forgetting, so a recycled instance id cannot inherit an elapsed reveal.
	ok += _expect(w.tracked() == 2, "tracks both threats before pruning")
	w.forget_missing([3])
	ok += _expect(w.tracked() == 1, "prunes threats that no longer exist")
	w.observe(2, 10.0, 0.25, 0.99, 0.0)
	ok += _expect(not w.visible(2, 10.1), "a recycled id starts a fresh reaction clock")
	return ok
